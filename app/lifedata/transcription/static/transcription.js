...
$("#syncallbtnid").click(function () {
  ...
  if (!sourceScript || !targetScripts) {
    proceed = false;
    alert('Please specify at least one source and target scripts');
  }

  if (targetScripts.includes(sourceScript)) {
    proceed = false;
    alert('Source and target script cannot be same');
  }

  if (sourceScript == 'Latin') {
    proceed = false;
    alert('Latin source is currently not suppported');
  }
  ...
  if (proceed) {
    ...
    transcriptionData['overwrite'] = overwrite;

    // console.log('Source script', sourceScript);
    // console.log('Target script', targetScripts);
    ...
    $.post("/savetranscription", {
      ...
      .done(function (data) {
        // console.log(data.savedTranscription);
        // if (!data.savedTranscription) {
        //   alert("Unable to sync the transcription as audio seem to be deleted or revoked access by one of the shared user. Showing you the next audio in the list.")
        //   window.location.reload();
        // }
        // else {
        //   alert("Transcription saved successfully.")
...
$('#vadalgorithmboundarydropdown').select2({
  // tags: true,
  placeholder: 'select algorithm',
  // data: posCategories
  // allowClear: true
...
$("#audiofile").change(function () {
  ...
  if (!(zipFileSize <= 500000000)) {

    const size = (zipFileSize / 1000 / 1000).toFixed(2);
    // console.log(zipFileSize, size);
    alert('Please upload file upto 500 MB. This file size is: ' + size + " MB");
