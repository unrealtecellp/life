        ...
        $.getJSON('/progressreport', {
            ...
            if (progressreport !== '') {
                ...
                progressreporttable += '<table class="table table-hover table-responsive text-center" id="myTable">'+
                                        ...
                                        '<th class="text-center" onclick="sortTable(4)">Completed Files</th>'+
                                        '<th class="text-center" onclick="sortTable(5)">Remaining Files</th>'+
                                        // '<th>View</th>'+
                                        // '<th>Edit</th>'+
                                        // '<th>Delete</th>'+
                                        '</tr></thead>';
                ...
                for (let i=0; i<progressreport.length; i++) {
                    oneRow = progressreport[i]
                    progressreporttable += '<tr>';
                    for (let [colName, colValue] of Object.entries(oneRow)) {
                        console.log(colName, colValue);
                        progressreporttable += '<td id="">'+colValue+'</td>';
                    }
                    progressreporttable += '</tr>';
                }
