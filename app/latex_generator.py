...
def get_latex_entries(all_entries, fields, dict_headword, char_list, doc):
    # class Multicols(Environment):
    #     """A class to wrap LaTeX's alltt environment."""
    ...
    for character in char_list:
        ...
        with doc.create(Section(character, numbering=False)):
    ...
    for field in fields:
        ...
        if type(field) == list:
            ...
            for i in range(max_entries):
                ...
                for field_type in field:
                    base_parts = field_type.split('.')                    
                    rest_parts = '.'.join(base_parts[1:])
                    new_field_name = base_parts[0]+'.'+insert_txt + '.' + rest_parts
...
def get_relevant_data(lexicon, fields, dict_headword):
    print ('Fields', fields)
...
def generate_formatted_latex(
    ...
    # \testfeature{Script=Tamil}{\tamiltext}
        ...
        doc.preamble.append(Command('fancyhead', arguments=Command('textsf', arguments=Command('rightmark')), options=Options('L')))
        doc.preamble.append(Command('fancyhead', arguments=Command('textsf', arguments=Command('leftmark')), options=Options('R')))
        doc.preamble.append(Command('renewcommand', arguments=Command('headrulewidth'), extra_arguments='1.4pt'))
        doc.preamble.append(Command('fancyfoot', arguments=Command('textbf', arguments=Command('textsf', arguments=Command('thepage'))), options=Options('C')))
        doc.preamble.append(Command('renewcommand', arguments=Command('footrulewidth'), extra_arguments='1.4pt'))

        doc_style = Command("pagestyle", arguments="fancy")
        doc.preamble.append(doc_style)

        headwordscript = lexicon[0]['langscripts']['headwordscript']
        otherscript = lexicon[0]['langscripts']['lexemeformscripts']
    ...
    # \pagestyle{fancy} % Use the custom headers and footers 
        ...
        with doc.create(Center()):
            ...
            \Huge'''))
            # \newline
            doc.append(NoEscape(r'''\center \textbf{A Dictionary of '''+project.title().replace('_', '\_')+'''}'''))
        ...
        required_entries, character_list = get_relevant_data(lexicon, expanded_fields, dict_headword)
        doc = get_latex_entries(required_entries, expanded_fields, dict_headword, character_list, doc)
        # doc.append(doc)
    ...
    if (rdf_format in file_ext_map) or (output_format in file_ext_map):
        if output_format == 'rdf':
            ...
            if output_format == 'csv':
                ...
                generate_formatted_latex(write_file, lexicon, project, fields=fields)
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(working_dir):
        working_dir = '/media/ritesh/Oldie/Dropbox/kmi-my-project-stuffs/LiFE/json_to_rdf'
    with open(os.path.join(working_dir, 'lexemeEntry.json')) as f_r:
        lex = json.load(f_r)
        out_form = 'latex_dict'
        rdf_form = 'xml'
        download_lexicon(lex, working_dir, out_form, rdf_form)
